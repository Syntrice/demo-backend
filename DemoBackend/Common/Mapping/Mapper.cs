...
public class Mapper : IMapper
{
    ...
    {
        Expression<Func<TSource, TDest>> expr = GetExpression<TSource, TDest>();
    ...
    private Expression<Func<TSource, TDest>> GetExpression<TSource, TDest>()
    {
        ...
        if (value == null)
        {
            throw new InvalidOperationException($"No map found for type {typeof(TSource)} to {typeof(TDest)}");
        }
